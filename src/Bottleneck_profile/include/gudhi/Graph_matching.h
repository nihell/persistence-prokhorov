...
class Graph_matching {
 public:
  ...
  void set_r(double r);
  /** \internal \brief Returns the number of currently unmatched edges. */
  int number_unmatched() const;
 ...
 private:
  ...
  /** \internal \brief All the unmatched points in V. */
  std::unordered_set<int> unmatched_in_v;
...
inline Graph_matching::Graph_matching(Persistence_graph& g)
    : gp(&g), r(0.), v_to_u(g.size(), null_point_index()), unmatched_in_u(g.size()), unmatched_in_v(g.size()) {
  for (int u_point_index = 0; u_point_index < g.size(); ++u_point_index)
    unmatched_in_u.insert(u_point_index);
    unmatched_in_v.insert(u_point_index);
}
...
inline int Graph_matching::number_unmatched() const {
  return unmatched_in_u.size()+unmatched_in_v.size();
}
...
inline void Graph_matching::update(std::vector<int>& path) {
  ...
  unmatched_in_v.erase(path.back());
